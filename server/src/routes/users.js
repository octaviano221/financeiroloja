...
function ensureAdmin(req, res, next) {
  if (req.user.role !== "ADMIN") {
    return res.status(403).json({ message: "Apenas administrador pode gerenciar usuários." });
  }
  return next();
}
...
router.post("/", async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ message: "Usuário inválido.", issues: parsed.error.issues });
...
router.patch("/:id/toggle", async (req, res) => {
  const id = Number(req.params.id);
  if (id === req.user.id) return res.status(400).json({ message: "Você não pode desativar seu próprio usuário." });
